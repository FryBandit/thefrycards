...
const Card: React.FC<CardProps> = ({ 
    ...
    isPotentialAttacker = false, onMouseEnter, onMouseLeave,
}) => {
  const [damageAnimClass, setDamageAnimClass] = useState('');
  const prevDamage = useRef(card.damage);

  useEffect(() => {
    if (card.damage > prevDamage.current) {
        setDamageAnimClass('animate-flash-damage');
        const timer = setTimeout(() => setDamageAnimClass(''), 500); // Match animation duration
        return () => clearTimeout(timer);
    }
    prevDamage.current = card.damage;
  }, [card.damage]);
  ...
  const isClickToExamine = !onClick && !!onExamine;
  ...
  const hoverScaleClass = isActionable && !inHand ? 'hover:scale-105' : '';
  ...
  const cardSizeClasses = inHand ? 'w-36 h-52' : 'w-36 h-48 md:w-48 md:h-64';
    ...
    <div className={`relative ${cardSizeClasses}`}>
      ...
      {((onActivate && !inHand) || (onChannel && inHand) || (onAmplify && inHand)) && (
        <div className="absolute -bottom-4 left-0 right-0 flex justify-center space-x-1 z-20">
            ...
            {onChannel && inHand && (
                 <button onClick={(e) => { e.stopPropagation(); onChannel(); }} disabled={!isChannelable} className={`px-3 py-0.5 text-xs font-bold rounded-full transition-all uppercase tracking-wider ${isChannelable ? 'bg-vivid-cyan text-arcane-bg shadow-vivid-cyan hover:scale-105' : 'bg-gray-800 text-gray-500 cursor-not-allowed'}`}>Channel</button>
            )}
            {onAmplify && inHand && (
